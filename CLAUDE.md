...
```
:domain                the platform-free core (spec module-architecture), born in migration step 3a: zone packages app.snapsync.model (vocabulary + pure codecs — the config surface incl. the EventLink codec + generated LINK_ORIGIN, sync vocabulary, selection policy + denylist + upload keys + device manifest + RawAsset mapping, edge-URL builder, SyncStatus/SyncProgress, PermissionStatus, and the Logger.invocation enter/exit helper — since step-8 C1 it drives the injected ports/LogScope seam; the LogContext ambient global was repaid to :adapter:ios:ext-safe), app.snapsync.ports (every port seam: config, keychain, gallery enumeration/status/manifest, permission, download + download-store, backend-need clients, upload platform/scheduler/discovery, push, attest, join marker, LedgerStore — seated here at step 5 per 3a's D3 — and, step 6, the album seams AlbumManager/AlbumMapStore), and the feature zones (steps 5–6) app.snapsync.feature.upload (UploadCycle + cycle gate, UploadArm/UploadProducer, BackgroundUploadPump, clearRequestedOffMain, ExtensionReconciler, SyncEngine + LedgerWriter — the single ledger-writer feature — and UploadPushReceiver, the upload arm's OS-callback receive seam re-homed from :capability:upload at step 8), feature.membership (JoinEvent + DeviceEnroller, LeaveEvent, and — step 6, single-writer of the manifest object behind Enrollment — DeviceManifestProducer), feature.status (SyncStatusSource + LedgerBackedSyncStatusSource + LedgerCountsSource + OwnDeviceGalleryStatusSource), feature.trust (DeviceAttestation), feature.download (DownloadController + QueuedPhotoDownloadJobs + DownloadPushReceiver + the DownloadStatusSource read-model and its store-backed impl — step 6), feature.album (AlbumCoordinator + the albumMapSource migration decision — step 6), feature.creation (CreateEvent use-case + the CreationStatus/EventCreator seams — step 6), and — step 7 — app.snapsync.compose: the SHARED composition (law "One shared composition"): uploadCore(scope, UploadPorts) → UploadCycle (the ONE cycle assembly — both device tiers' roots AND the world call it; owns the unified port-pure readGate, D1 of establish-shared-composition: the app-driven tier's per-cycle ConfigSource-StateFlow reload was dropped, the unlock hook owns that repair), snapSyncApp(scope, AppPorts) → AppCore (the app feature graph: attestation, upload arm, join/leave/create, downloads, status read-models; also — step 8 — the flow instances + the permission-grant subscriptions built/installed here over the compose scope; tier selection stays shell-supplied until C3's resolveComposition switch), and ResourceEnumerator (the PhotoLibrary-from-RawAssetSource port composition, step-6 D4 repaid); and — step 8 — app.snapsync.flow: the OS-callback trigger flows (Foreground · Background · SilentPush [the cross-arm push fan-out, absorbing FanOutPushReceiver] · DownloadBackstop · Provision), each importing model/+feature/ ONLY (flow-no-ports gate now armed), every port/platform touch injected as a compose/-built effect lambda. Zero project() deps; jvm+iosArm64+iosSimulatorArm64; no iosMain source dir; model-purity + ports→model + feature-blindness + flow-no-ports gates armed in :test:architecture
...
:app:ios               iOS app wiring + framework export (thin, untested); SnapSyncRoot builds the platform adapters and calls :domain compose/'s snapSyncApp (step 7)
