...
:app:ios:photokit-extension  iOS ≥26.1 background-upload extension: PhotoKit adapter (IosPhotoKitUploadPlatform) + composition root calling :domain compose/'s uploadCore (step 7), composing :adapter:ios:ext-safe + :adapter:generic (the SnapSyncUploadKit framework) — thin, untested (orchestration + tests live in :domain feature/upload; dies at 13a-rename)
