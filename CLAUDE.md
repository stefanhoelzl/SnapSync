...
desktop harnesses since migration step 10): the real
...
```
:domain                the platform-free core (spec module-architecture), born in migration step 3a: zone packages app.snapsync.model (vocabulary + pure codecs — the config surface incl. the EventLink codec + generated LINK_ORIGIN, sync vocabulary, selection policy + denylist + upload keys + device manifest + RawAsset mapping, edge-URL builder, SyncStatus/SyncProgress, PermissionStatus, and the Logger.invocation enter/exit helper — since step-8 C1 it drives the injected ports/LogScope seam; the LogContext ambient global was repaid to :adapter:ios:ext-safe), app.snapsync.ports (every port seam: config, keychain, gallery enumeration/status/manifest, permission, download + download-store, backend-need clients, upload platform/scheduler/discovery, push, attest, join marker, LedgerStore — seated here at step 5 per 3a's D3 — and, step 6, the album seams AlbumManager/AlbumMapStore), and the feature zones (steps 5–6) app.snapsync.feature.upload (UploadCycle + cycle gate, UploadArm/UploadProducer, BackgroundUploadPump, clearRequestedOffMain, ExtensionReconciler, SyncEngine + LedgerWriter — the single ledger-writer feature — and UploadPushReceiver, the upload arm's OS-callback receive seam re-homed from :capability:upload at step 8), feature.membership (JoinEvent + DeviceEnroller, LeaveEvent, DeviceManifestProducer — step 6, single-writer of the manifest object behind Enrollment — and EventName.storeEventNameIfChanged, the step-8 C3 name-refresh rule sink), feature.status (SyncStatusSource + LedgerBackedSyncStatusSource + LedgerCountsSource + OwnDeviceGalleryStatusSource), feature.trust (DeviceAttestation), feature.download (DownloadController + QueuedPhotoDownloadJobs + DownloadPushReceiver + the DownloadStatusSource read-model and its store-backed impl — step 6), feature.album (AlbumCoordinator — since step-8 C3 its ensureAlbum owns the membership opt-in gate and albumIdFor the import-time lookup — + the albumMapSource migration decision, step 6), feature.creation (CreateEvent use-case + the CreationStatus/EventCreator seams — step 6), and — step 7 — app.snapsync.compose: the SHARED composition (law "One shared composition"): uploadCore(scope, UploadPorts) → UploadCycle (the ONE cycle assembly — both device tiers' roots AND the world call it; owns the unified port-pure readGate, D1 of establish-shared-composition: the app-driven tier's per-cycle ConfigSource-StateFlow reload was dropped; since step 12 the trigger flows' reloadConfig re-read owns that repair), snapSyncApp(scope, AppPorts) → AppCore (the app feature graph: attestation, upload arm, join/leave/create, downloads, status read-models; also — step 8 — the flow instances, the flow/ UserCommands user-tap bundle [leave·create·commitJoin·share, injected into StatusContainerHost — presentation references no feature command], and the permission-grant subscriptions as an explicit installPermissionSubscriptions() the app shell invokes from host assembly ONLY — a cold background wake that merely touches AppCore starts no producer; tier selection is the shell's one resolveComposition switch since C3), and ResourceEnumerator (the PhotoLibrary-from-RawAssetSource port composition, step-6 D4 repaid); and — step 8 — app.snapsync.flow: the OS-callback trigger flows (Foreground · Background · SilentPush [the cross-arm push fan-out, absorbing FanOutPushReceiver] · DownloadBackstop · Provision), each importing model/+feature/ ONLY (flow-no-ports gate now armed), every port/platform touch injected as a compose/-built effect lambda. Zero project() deps; jvm+iosArm64+iosSimulatorArm64; no iosMain source dir; model-purity + ports→model + feature-blindness + flow-no-ports gates armed in :test:architecture
:adapter:generic       platform-free technology impls of the :domain ports (migration step 4): the Ktor clients (HttpAttestClient, HttpEnrollment, HttpEventDirectory, HttpDeviceFilesSource, HttpLeaveNotifier, HttpEventUnionSource, HttpEventCreation, KtorPushHttpClient) + the SQLDelight stores (SqlDelightLedgerStore, SqlDelightDownloadStore) with both db schemas; jvm+ios. Packages keep their pre-step-4 names (pure-move decision, D2 of extract-adapter-modules). Its jvmTest/iosSimulatorArm64Test extend the storage contracts from :test:world (the driver-backed tests re-homed here at step 10)
:adapter:ios:ext-safe  every iOS adapter the EXTENSION process links (step 4; placed by linkage): the discovery walk + cursor store (IosDiscovery/IosDiscoveryStore), ledger/download-store native drivers (iosLedgerStore/iosDownloadStore + the App-Group consts), manifest store + PhotoKit enumerator, device-log writers (FileLogWriter/PublicNSLogWriter), darwinHttpClient, IosJoinedEventMarker, IosAlbumManager/IosAlbumMapStore, IosAttestKey, FileBackedConfigStore (the App-Group config file of record since step 11a, composing KeychainConfigStore as its written-through fallback until 13b) — and the Keychain impls: this is now the ONLY module that may touch SecItem* (IosKeychain, KeychainDeviceIdentity, KeychainConfigStore, KeychainAttestStore; the :test:architecture guard enforces it, and the extension-safety gate forbids platform.UIKit/BackgroundTasks anywhere in it)
:adapter:ios:app-only  iOS adapters only the MAIN APP process links (step 4): IosUrlSessionUploadPlatform + IosBackgroundScheduler (the 18–26.0 tier), IosDownloadTransport, IosPhotoLibraryImporter, PhotoLibraryPermission — the two URLSession adapters own OS-reattached app-process session ids, so an extension-side link must stay structurally impossible
:adapter:fake          the honest in-memory port impls (migration step 10; package app.snapsync.fake): InMemoryLedgerStore/DownloadStore/AttestStore/PhotoLibrary/GalleryStatusSource/RawAssetSource + the discovery/marker/manifest/album-map stores — what the world harness and every integration test stand on. Honesty is MECHANICAL (FakeHonestyTest, armed here): a fake's public surface is its port contract plus a constructor taking initial state (the three gallery fakes take a MutableStateFlow cell); operator rigging (levers, inspection) lives in :test:world wrappers (WorldGallery, RecordingDownloadStore), never here. jvm+iosSimulatorArm64 only (never links into a shipped framework); commonTest hosts the re-homed fake-driven feature tests (RawAssetMapping, status sources, download trio, DeviceAttestation)
...
:app:desktop           the ONE desktop module (step 10: :app:desktop:ui folded in): the shared pane library (PhoneFrame + StatusPane, StatusContainerHost wiring) + BOTH harness apps — the full-stack world harness (:app:desktop:run; real StatusScreen whose counts EMERGE from the AppCore the world composes via snapSyncApp, + a right-pane world inspector; capability full-stack-harness) and the forge harness (:app:desktop:runForge, a JavaExec — the Compose plugin models one application main class; phone frame + control panel forging any UI state; capability desktop-test-harness)
:app:ios               iOS app wiring + framework export (thin, untested); SnapSyncRoot parses LaunchDirectives, resolves the sealed CompositionMode ONCE (model/'s resolveComposition — forge excludes the live-stack boot structurally: the one when(mode) picks a ForgeShell/LiveShell delegate and ForgeShell holds no route to app/host), builds the platform adapters and calls :domain compose/'s snapSyncApp (steps 7–8)
:app:ios:extension     iOS ≥26.1 background-upload extension: PhotoKit adapter (IosPhotoKitUploadPlatform) + composition root calling :domain compose/'s uploadCore (step 7), composing :adapter:ios:ext-safe + :adapter:generic (the SnapSyncUploadKit framework) — thin, untested (orchestration + tests live in :domain feature/upload; renamed from :app:ios:photokit-extension at step 13a)
:test:world            test-only shared infra: the controllable in-memory "world" — BackendStore + MockEngine mini-edge + operator levers/wrappers rigging :adapter:fake's honest doubles — whose World.core IS the real AppCore from the SAME snapSyncApp the iOS shell calls (step 10; features, flows, and the UserCommands bundle are production instances), with the extension-tier cycle from the same uploadCore (step 7). commonMain also hosts the LedgerStore/DownloadStore CONTRACTS (a test source set cannot be exported; :adapter:generic's driver tests and the fake-backed runs extend them from their own test source sets). jvm()+iosSimulatorArm64. Consumed by :app:desktop AND :test:integration (capability harness-world-model)
:test:architecture     test-only JVM guards for invariants the compiler cannot express (capability architecture-guards): Konsist — no SecItem* outside :adapter:ios:ext-safe (catches fully-qualified calls, which no linter can see on iosMain); the extension-safety gate (no platform.UIKit/BackgroundTasks in extension-linked source); plus the entitlements never raise default-data-protection to NSFileProtectionComplete (which would make every App-Group file unreadable while locked, killing the background tier)
...
- Log via **Kermit** (multiplatform). Cross-cutting logging infra lives in **`:domain`'s `model/`**
  (since migration step 5; `:domain:logging` died at step 6): the `Logger.invocation` enter/exit
  helper (params + result + duration) and the process-global `LogContext` ambient prefix. The
  consolidated iOS device-log writers (`FileLogWriter`/`PublicNSLogWriter`) live in
  `:adapter:ios:ext-safe` (migration step 4).
