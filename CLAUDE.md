...
:adapter:generic       platform-free technology impls of the :domain ports (migration step 4): the Ktor clients (HttpAttestClient, HttpEnrollment, HttpEventDirectory, HttpDeviceFilesSource, HttpLeaveNotifier, HttpEventUnionSource, HttpEventCreation, KtorPushHttpClient) + the SQLDelight stores (SqlDelightLedgerStore, SqlDelightDownloadStore) with both db schemas; jvm+ios. Packages keep their pre-step-4 names (pure-move decision, D2 of extract-adapter-modules)
...
:domain:engine         tests-only since step 4 (LedgerStoreContract + InMemory backend + SyncEngineTest; the SqlDelight store + schema live in :adapter:generic, the native driver factory in :adapter:ios:ext-safe; the driver-backed tests stay here because they extend the contract, which a test source set cannot export)
...
:test:world            test-only shared infra: a controllable in-memory "world" (backend object store + read-models, MockEngine mini-edge, operator-driven BackgroundTransfer/download fakes) the REAL stack runs against + a cycle assembled by the SAME :domain compose/ uploadCore the device tiers call (step 7; full snapSyncApp collapse at step 10); jvm()+iosSimulatorArm64. Consumed by :app:desktop AND :test:integration (capability harness-world-model)
