...
```
:domain                the platform-free core (spec module-architecture), born in migration step 3a: zone packages app.snapsync.model (vocabulary + pure codecs — the config surface incl. the EventLink codec + generated LINK_ORIGIN, sync vocabulary, selection policy + denylist + upload keys + device manifest + RawAsset mapping, edge-URL builder, SyncStatus/SyncProgress, PermissionStatus, and — step-5 interim, dissolves into step-8 compose/ decorators — the Logger.invocation helper + LogContext ambient prefix), app.snapsync.ports (every port seam: config, keychain, gallery enumeration/status/manifest, permission, download + download-store, backend-need clients, upload platform/scheduler/discovery, push, attest, join marker, LedgerStore — seated here at step 5 per 3a's D3 — and, step 6, the album seams AlbumManager/AlbumMapStore), and the feature zones (steps 5–6) app.snapsync.feature.upload (UploadCycle + cycle gate, UploadArm/UploadProducer, BackgroundUploadPump, clearRequestedOffMain, ExtensionReconciler, SyncEngine + LedgerWriter — the single ledger-writer feature — + the ResourceEnumerator port composition, step 6), feature.membership (JoinEvent + DeviceEnroller, LeaveEvent, and — step 6, single-writer of the manifest object behind Enrollment — DeviceManifestProducer), feature.status (SyncStatusSource + LedgerBackedSyncStatusSource + LedgerCountsSource + OwnDeviceGalleryStatusSource), feature.trust (DeviceAttestation), feature.download (DownloadController + QueuedPhotoDownloadJobs + DownloadPushReceiver + the DownloadStatusSource read-model and its store-backed impl — step 6), feature.album (AlbumCoordinator + the albumMapSource migration decision — step 6), feature.creation (CreateEvent use-case + the CreationStatus/EventCreator seams — step 6). Zero project() deps; jvm+iosArm64+iosSimulatorArm64; no iosMain source dir; model-purity + ports→model + feature-blindness gates armed in :test:architecture
