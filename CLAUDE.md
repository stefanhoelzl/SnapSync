...
:domain:logging        cross-cutting diagnostics: Logger.invocation helper + LogContext ambient prefix (Kermit-only leaf; the iOS device-log writers moved to :adapter:ios:ext-safe, step 4) (capability diagnostic-logging)
...
:capability:join       join use-case + DeviceEnroller (writes the per-event device manifest = the physical fact of membership) over the EventDirectory port (join-event; the HTTP clients moved to :adapter:generic, step 4)
...
:capability:membership event-membership lifecycle: extension-side re-join reconciliation + leave use-case + device-file listing seam (the HTTP clients + Darwin client + joined-event marker moved to the adapter modules, step 4)
