...
```
:domain                the platform-free core (spec module-architecture), born in migration step 3a: zone packages app.snapsync.model (vocabulary + pure codecs — the config surface incl. the EventLink codec + generated LINK_ORIGIN, sync vocabulary, selection policy + denylist + upload keys + device manifest + RawAsset mapping, edge-URL builder, SyncStatus/SyncProgress, PermissionStatus, and — step-5 interim, dissolves into step-8 compose/ decorators — the Logger.invocation helper + LogContext ambient prefix), app.snapsync.ports (every port seam: config, keychain, gallery enumeration/status/manifest, permission, download + download-store, backend-need clients, upload platform/scheduler/discovery, push, attest, join marker, LedgerStore — seated here at step 5 per 3a's D3), and the step-5 feature zones app.snapsync.feature.upload (UploadCycle + cycle gate, UploadArm/UploadProducer, BackgroundUploadPump, clearRequestedOffMain, ExtensionReconciler, SyncEngine + LedgerWriter — the single ledger-writer feature), feature.membership (JoinEvent + DeviceEnroller, LeaveEvent), feature.status (SyncStatusSource + LedgerBackedSyncStatusSource + LedgerCountsSource + OwnDeviceGalleryStatusSource), feature.trust (DeviceAttestation). Zero project() deps; jvm+iosArm64+iosSimulatorArm64; no iosMain source dir; model-purity + ports→model + feature-blindness gates armed in :test:architecture
...
:domain:logging        SOURCELESS since step 5 (Logger.invocation + LogContext moved to :domain model/; the iOS device-log writers moved to :adapter:ios:ext-safe at step 4); skeleton dies at step 6 (capability diagnostic-logging)
:domain:status         DownloadStatusSource only since step 5 (the download arm's read-model — moves with the download feature at step 6; the ledger→SyncStatus projections moved to :domain feature/status) + two stay-behind tests driving the moved projections through :domain:gallery's fakes
:domain:permission     EMPTIED by step 4 (PhotoLibraryPermission → :adapter:ios:app-only; the PhotoAccess* ports live in :domain since 3a); skeleton dies at steps 5/6
:domain:gallery        producer/enumerator composition + in-memory fakes (the iOS PhotoKit adapters moved to :adapter:ios:ext-safe, step 4; step 3a moved the port seams and the selection policy's vocabulary to :domain)
:domain:download-store  the in-memory download store + DownloadStoreContract (the SqlDelight store moved to :adapter:generic, the iOS driver factory to :adapter:ios:ext-safe, step 4; echo-suppression projection rides the store)
...
:capability:upload     UploadPushReceiver only since step 5 (the OS-callback receive seam — flow material for step 8; the upload orchestration moved to :domain feature/upload)
...
:capability:config     EMPTIED by step 4 (KeychainConfigStore → :adapter:ios:ext-safe; the codec + config surface moved to :domain at 3a); skeleton dies at steps 5/6 (capability event-link)
...
:capability:download   foreign-photo download → stage → import controller (photo-download; the HTTP union source and the iOS transport/importer moved to the adapter modules, step 4)
:capability:join       SOURCELESS since step 5 (join use-case + DeviceEnroller moved to :domain feature/membership; the HTTP clients moved to :adapter:generic at step 4); skeleton dies at step 6
:capability:album      tested commonMain album orchestration: resolve-or-create the per-event album, dispatch-or-skip an add; PhotoKit behind seams (event-album; the iOS impls moved to :adapter:ios:ext-safe, step 4; the DENYLIST moved to :domain, step 3a; capability photo-selection-policy)
...
:capability:membership SOURCELESS since step 5 (the re-join reconciliation + clearRequestedOffMain moved to :domain feature/upload — single-writer placement — and the leave use-case to feature/membership; the HTTP clients + Darwin client + joined-event marker moved to the adapter modules at step 4); skeleton dies at step 6
...
:app:ios:photokit-extension  iOS ≥26.1 background-upload extension: PhotoKit adapter (IosPhotoKitUploadPlatform) + composition root, composing :capability:upload + :adapter:ios:ext-safe + :adapter:generic (the SnapSyncUploadKit framework) — thin, untested (orchestration + tests live in :capability:upload; dies at 13a-rename)
...
- Log via **Kermit** (multiplatform). Cross-cutting logging infra lives in **`:domain:logging`**: the
  `Logger.invocation` enter/exit helper (params + result + duration) and the process-global
  `LogContext` ambient prefix. The consolidated iOS device-log writers
  (`FileLogWriter`/`PublicNSLogWriter`) live in `:adapter:ios:ext-safe` (migration step 4).
