...
:adapter:generic:app   platform-free technology impls of the :domain ports: the Ktor clients (HttpAttestClient, HttpEventJoin, HttpManifestPublisher, HttpEventDirectory, HttpDeviceFilesSource, HttpLeaveNotifier, HttpEventUnionSource, HttpEventCreation, HttpPushTokenPublisher) + the shared cross-cutting HTTP interceptor (`withCredentialInterceptor`: the device token, the declared app version, the 401 recovery loop, the 426 refusal) + the SQLDelight stores (SqlDelightLedgerStore, SqlDelightDownloadStore) with both db schemas + the SystemClock/SystemTimeZone port impls; jvm+ios. Its jvmTest/iosSimulatorArm64Test bind the storage contracts from :test:contracts; its jvmTest also holds LiveEdge (the real api/ served by a deno process per test JVM) and the backend contracts' Live bindings over the real Ktor clients
