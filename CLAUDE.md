...
```
:domain                the platform-free core (spec module-architecture), born in migration step 3a: zone packages app.snapsync.model (vocabulary + pure codecs — the config surface incl. the EventLink codec + generated LINK_ORIGIN, sync vocabulary, selection policy + denylist + upload keys + device manifest + RawAsset mapping, edge-URL builder, SyncStatus/SyncProgress, PermissionStatus, and — step-5 interim, dissolves into step-8 compose/ decorators — the Logger.invocation helper + LogContext ambient prefix), app.snapsync.ports (every port seam: config, keychain, gallery enumeration/status/manifest, permission, download + download-store, backend-need clients, upload platform/scheduler/discovery, push, attest, join marker, LedgerStore — seated here at step 5 per 3a's D3 — and, step 6, the album seams AlbumManager/AlbumMapStore), and the feature zones (steps 5–6) app.snapsync.feature.upload (UploadCycle + cycle gate, UploadArm/UploadProducer, BackgroundUploadPump, clearRequestedOffMain, ExtensionReconciler, SyncEngine + LedgerWriter — the single ledger-writer feature), feature.membership (JoinEvent + DeviceEnroller, LeaveEvent, and — step 6, single-writer of the manifest object behind Enrollment — DeviceManifestProducer), feature.status (SyncStatusSource + LedgerBackedSyncStatusSource + LedgerCountsSource + OwnDeviceGalleryStatusSource), feature.trust (DeviceAttestation), feature.download (DownloadController + QueuedPhotoDownloadJobs + DownloadPushReceiver + the DownloadStatusSource read-model and its store-backed impl — step 6), feature.album (AlbumCoordinator + the albumMapSource migration decision — step 6), feature.creation (CreateEvent use-case + the CreationStatus/EventCreator seams — step 6), and — step 7 — app.snapsync.compose: the SHARED composition (law "One shared composition"): uploadCore(scope, UploadPorts) → UploadCycle (the ONE cycle assembly — both device tiers' roots AND the world call it; owns the unified port-pure readGate, D1 of establish-shared-composition: the app-driven tier's per-cycle ConfigSource-StateFlow reload was dropped, the unlock hook owns that repair), snapSyncApp(scope, AppPorts) → AppCore (the app feature graph: attestation, upload arm, join/leave/create, downloads, status read-models; coordination lambdas + tier selection stay shell-supplied until step 8's flow//resolveComposition), and ResourceEnumerator (the PhotoLibrary-from-RawAssetSource port composition, step-6 D4 repaid). Zero project() deps; jvm+iosArm64+iosSimulatorArm64; no iosMain source dir; model-purity + ports→model + feature-blindness gates armed in :test:architecture
...
:capability:upload     UploadPushReceiver + FanOutPushReceiver only since step 5 (the OS-callback receive seam and the cross-arm fan-out — flow/compose material for steps 7–8, which is why this module outlived step 6's deletions; the upload orchestration moved to :domain feature/upload)
...
:app:ios:photokit-extension  iOS ≥26.1 background-upload extension: PhotoKit adapter (IosPhotoKitUploadPlatform) + composition root calling :domain compose/'s uploadCore (step 7), composing :capability:upload + :adapter:ios:ext-safe + :adapter:generic (the SnapSyncUploadKit framework) — thin, untested (orchestration + tests live in :domain feature/upload; dies at 13a-rename)
