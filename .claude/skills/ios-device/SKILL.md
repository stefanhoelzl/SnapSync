...
description: >-
  ...
  screenshot it, pull its logs, and verify that uploads and event links really
  landed. Use whenever the task touches the physical device: "install on the
  phone", "launch the app", "take a screenshot", "read debug.log", "test on the
  SE2", or any pymobiledevice3/dvt/libimobiledevice command. To DRIVE a running
  app — join, create, leave, reset, seed, wipe, read the gallery — load
  `rig-channel` instead; this skill stops at the running app.
---
...
Dev IPAs come from the interactive dev build loop (`ssh-mac-build`). There is **no CI dev-IPA artifact**; CI
...
## Driving the app — that is the channel's job now

There is no launch-trigger index here any more, because there are no launch triggers. Joining, creating,
leaving, resetting, seeding, wiping and reading the selection policy are **control-channel** calls against a
running app: load the **`rig-channel`** skill.

The boundary between the two runbooks is the running app. This one gets a build onto the phone and evidence
back off it — lease, install, launch, screenshot, restart, log pull, and confirming an event link resolves.
Everything you do **to** an app that is already running is `rig-channel`.

⚠️ **A build without `-Psnapsync.rig=true` is undriveable, and that is deliberate.** The dev surface no
longer ships: production Kotlin declares no `SNAPSYNC_*` variable at all, and a guard fails the build if one
returns. You can still install and launch a release build and watch it — you just cannot make it do
anything. Build with the property when you intend to drive it.
...
rig build, read it through the channel instead — `GET /device/logs?process=extension` — which needs no
copy step, no relaunch and no pull (see the `rig-channel` skill). One reduction against the old copy-out
trigger: the channel reads the **current** file only, so a rolled `.1` sibling is not reachable that way. ⚠️ `apps pull app.snapsync.BackgroundUpload Documents/debug.log` is **dead**: the extension
...
`ssh-mac-build` builds the dev IPA **with `snapsync.rig=true`** → `apps install` → `dvt launch` → join
