 ...
 * `ios-url-session-upload`) — its two tail units, its heartbeat, and its session.
 *
 ...
 * process may create (capability `upload-lifecycle`), so a unit a declining membership reaches still returns.
 */
interface AppUploadMechanism {
    ...
    /**
     * The tail's ③ — the walk and the manifest publish, abandoned on a stop (capability `ios-app-shell`, "The discovery
 ...
 * construction (spec `module-architecture`, "Callbacks are bound at construction"): the transport exists before the
