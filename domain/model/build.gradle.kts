...
val generateEventWindowMax by tasks.registering {
    ...
    outputs.dir(outDir)
    doLast {
        val file = outDir.get().asFile.resolve("app/snapsync/model/EventWindowMax.kt")
        file.parentFile.mkdirs()
        file.writeText(
            """
            // GENERATED by :domain's generateEventWindowMax task — do not edit.
            // Source of truth: the resolved deployment's `eventWindowMaxSeconds` (see deployments/ and
            // scripts/resolve-deployment.py) — the value the backend validates `endsAt - startsAt` against.
            package app.snapsync.model
...
kotlin {
    ...
    sourceSets {
        commonMain.configure {
            kotlin.srcDir(generateLinkOrigin)
            kotlin.srcDir(generateEventWindowMax)
        }
