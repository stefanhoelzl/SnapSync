 ...
 * `sync-status`). Shown tracks completeness (work remains), pulsing tracks live activity —
 * each arrow derives from its own counts alone. Seated in `model/` (migration step 9, the
 * Arrow/ArrowLevel unification): the one declaration both `:domain:presentation` (`SyncHealth.Syncing`)
