...
class UploadArmTest {
    ...
    ) = UploadArm(producer, isGranted = { granted }, includesUpload = { includesUpload })
    ...
    fun provisioning_with_access_already_granted_starts_the_producer() = runTest {
        ...
        arm(producer, granted = true, includesUpload = true).onProvision()
    ...
    fun provisioning_a_download_only_membership_stops_the_producer() = runTest {
        ...
        arm(producer, granted = true, includesUpload = false).onProvision()
    ...
    fun provisioning_without_access_fires_neither_verb() = runTest {
        ...
        arm(producer, granted = false, includesUpload = true).onProvision()
    ...
    fun a_grant_starts_the_producer() = runTest {
        ...
        arm(producer, granted = true, includesUpload = true).onPermissionGranted()
    ...
    fun a_grant_on_a_download_only_membership_starts_nothing() = runTest {
        ...
        arm(producer, granted = true, includesUpload = false).onPermissionGranted()
    ...
    fun joining_before_the_grant_still_starts_once_access_arrives() = runTest {
        val producer = FakeProducer()
        var granted = false
        val a = UploadArm(producer, isGranted = { granted }, includesUpload = { true })
    ...
    fun no_transition_can_reach_a_destructive_verb() = runTest {
        val producer = FakeProducer()
        var granted = true
        var includesUpload = true
        val a = UploadArm(producer, isGranted = { granted }, includesUpload = { includesUpload })
