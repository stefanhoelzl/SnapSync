...
/**
 * The seam that fetches a **device's** already-stored object filenames (`GET /files/device/<deviceId>`,
 ...
 * composition root; `MockEngine` in tests). GETs `<host>/files/device/<deviceId>` (HTTPS, default
...
class HttpDeviceFilesSource(
    ...
    override suspend fun list(deviceId: String): Result<List<String>> = runCatching {
        val response = client.get("$base/files/device/$deviceId")
