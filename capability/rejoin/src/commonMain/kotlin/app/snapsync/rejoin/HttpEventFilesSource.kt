 ...
 * a failed [Result], and parses the flat array into [RemoteFile]s (ignoring the `deviceId`/`size`
 * fields the join does not need). The `eventId` is a UUID, so no path encoding is required.
 */
