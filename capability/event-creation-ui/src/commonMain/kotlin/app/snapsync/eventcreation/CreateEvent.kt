 ...
 * uses (capability `join-event`). `create(name, startsAt)` is fire-and-forget — it launches on the
 * injected [scope], sets [CreationStatus.InFlight], calls the backend via [client] with the trimmed name
 * and the canonical start date, and:
...
class CreateEvent(
    ...
    override fun create(name: String, startsAt: String) {
        scope.launch {
            ...
            when (val outcome = client.create(name.trim(), startsAt)) {
