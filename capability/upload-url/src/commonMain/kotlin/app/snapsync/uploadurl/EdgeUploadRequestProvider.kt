 ...
 * `<host>/devices/<deviceId>/files/<encoded-filename>` — **no network, no signing, no crypto, no auth
 ...
 * partition (`/devices/<deviceId>/files/`) and linked into any number of events by reference (the per-event
 ...
 * contract: `resource.filename → devices/<deviceId>/files/<percent-encoded filename>`. The URL is
...
class EdgeUploadRequestProvider(
    ...
    override suspend fun provide(resource: Resource): UploadRequest {
        val url = "$base/devices/$deviceId/files/${encodeFilenameSegment(resource.filename)}"
