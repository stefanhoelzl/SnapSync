 ...
 * `<host>/event/<eventId>/file/<encoded-filename>` — **no network, no signing, no crypto, no auth
 * header** (the edge endpoint authorizes by the `eventId` in the path) and **no custom metadata
 ...
 * contract: `resource.filename → <eventId>/<percent-encoded filename>`. The URL is **stable with no
 * expiry** — rebuilding for the same inputs yields a byte-identical URL, so a retry re-derived much
 * later re-PUTs the exact same destination (nothing to re-mint).
 *
 * [host]/[eventId] are plain strings, injected by the consuming composition root (host baked at
 * compile time, eventId from the Keychain). The provider makes no platform call.
 */
class EdgeUploadRequestProvider(
    ...
    override suspend fun provide(resource: Resource): UploadRequest {
        val url = "$base/event/$eventId/file/${encodeFilenameSegment(resource.filename)}"
