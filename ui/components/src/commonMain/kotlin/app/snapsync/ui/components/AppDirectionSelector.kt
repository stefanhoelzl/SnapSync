 ...
 * `Direction` to/from this, so the design system stays decoupled from the config capability. (The old
 * `Arrow`/`ArrowLevel` mirror of this pattern was unified into the one `model/` `Arrow` at migration
 * step 9 — the arrow enum is shared sync vocabulary, not a config-capability coupling.)
 */
