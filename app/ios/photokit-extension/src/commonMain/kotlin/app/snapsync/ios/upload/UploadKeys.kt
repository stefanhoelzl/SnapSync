 ...
 * `"<cloudId>-<kind>.<ext>"` layout (design.md §3.1) lives. Kept platform-free so the layout is
 * unit-tested on the simulator instead of trapped inside the PhotoKit adapter; the adapter only
 * supplies the raw fields.
 */
internal fun uploadKey(cloudId: String, resourceType: Long, originalFilename: String): String =
    "$cloudId-${resourceKind(resourceType)}.${fileExtension(originalFilename)}"
