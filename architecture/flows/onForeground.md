...
- line 552 — `if` conditional: `if (isForging) {`
- line 556 — bare property read (lazy-init touch): `host`
- line 563 — `if` conditional: `if (useAppDrivenUpload) urlSessionUpload.onForeground()`
- line 566 — `scope.launch` body: `scope.launch { refreshStatusSources() }`
- line 569 — `scope.launch` body: `scope.launch { config.config.value?.eventId?.let { downloadController.reconcile(it) } }`
- line 571 — `scope.launch` body: `scope.launch { config.config.value?.eventId?.let { fetchAndStoreName(it) } }`
