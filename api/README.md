...
site/index.html · site/join/index.html · site/_astro/*   the Astro build (public; mirror-deployed by deploy.yml's `site` job)
```
...
> never reads or writes the column. Decision record:
> `openspec/changes/archive/2026-09-22-manifest-versions`.
...
migrations/*.sql  schema EVOLUTION: ordered, checksummed files. The DEPLOYED store is migrated by
                  `bunny db migrations` (the platform runner) from deploy.yml; the rig, the test
...
src/scripts/      OUT-OF-EDGE programs (never bundled): sweep.ts (the nightly cleanup), migrate.ts
                  (applied by the api deploy before it publishes), probe.ts (the post-deploy boot probe).
...
`bunny db migrations` from deploy.yml's `api` job, **before** it publishes the bundle — a failed
migration fails the run with the previous bundle still live. Everything local uses
`src/dev/replay.ts`, because the CLI refuses any URL that is not `libsql://`/`https://`/`wss://` and
therefore cannot address a file or `:memory:`. Both apply each file with foreign-key enforcement
genuinely OFF (not deferred): `DROP TABLE` performs an implicit delete that FIRES
`ON DELETE CASCADE`, so a rebuild of a referenced table would otherwise empty its children while
reporting success. The rig migrates on start, so a `.localstore` from an older rig is carried
forward rather than needing a wipe.

THE DEPLOY ASSERTS THE RESULT. deploy.yml's `api` job compares the live store's `sqlite_master`
against `schema.sql` — before the window decision when no migration is pending (where a store
hand-edited through `bunny db shell` is caught, for free), and after applying when one is.
...
CI deploys via the `api` job of `.github/workflows/deploy.yml` (runs on `main` only, and only when
`api/**`, `deployments/**` or the resolver changed since the commit `/health` reports as live; the
`deno fmt`/`lint`/`check`/`test` gate is `api.yml`'s required `api-test`, which no commit reaches
`main` without) using `BunnyWay/actions/deploy-script`. It takes one of **two paths**, decided by
`migrate.ts --pending`:
...
> **The browser-facing site is a SEPARATE deploy.** The `site` job of `.github/workflows/deploy.yml`
> builds the Astro `site/` module (under Node) and **mirror-deploys** it to the storage `site/`
> prefix (`site/scripts/deploy.mjs` — upload new, delete stale, never clear-first), authenticating
> with **only the storage-zone password** (`BUNNY_STORAGE_ACCESS_KEY`), never the account key. The
> api Edge Script proxies that prefix, so the routing lives in the bundle as source-owned code —
> **no pull-zone edge rules**. It runs on every push to `main` (the mirror is idempotent); the
> site's **gate** is `.github/workflows/site.yml` (`site-build`: build + `npm run check` on every
> branch, no path filter, a required check), split from the deploy the same way and for the same
> reason as `api.yml`. Capability `web-site`.
