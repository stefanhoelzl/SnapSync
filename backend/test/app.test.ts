...
// It exists to measure what the pull zone and the OS put on the wire; these tests only pin that the
// instrument itself reports faithfully — a lying instrument is worse than none.

Deno.test("spike echo → reflects the headers observed AT THE ORIGIN", async () => {
  const { fetchImpl } = storageFake({});
  const res = await createApp({ config: CONFIG, fetch: fetchImpl }).request("/__spike/echo", {
    headers: { authorization: "Bearer probe-token", "x-snapsync-token": "probe-custom" },
  });

  assertEquals(res.status, 200);
  const { observedAtOrigin } = await res.json() as { observedAtOrigin: Record<string, string> };
  assertEquals(observedAtOrigin["authorization"], "Bearer probe-token");
  assertEquals(observedAtOrigin["x-snapsync-token"], "probe-custom");
  assertEquals(res.headers.get("Cache-Control"), "no-store, no-cache, max-age=0");
});

Deno.test("spike PUT → records the observed headers, drains the body, stores NO bytes", async () => {
  const { calls, fetchImpl } = storageFake({});
  const app = createApp({ config: CONFIG, fetch: fetchImpl });

  const res = await app.request(`/__spike/files/devices/${D}/IMG_0001-photo.jpg`, {
    method: "PUT",
    body: "pretend-photo-bytes",
    headers: { authorization: "Bearer probe-token", "content-type": "image/jpeg" },
  });
  assertEquals(res.status, 201);

  // Exactly one upstream write, and it is the RECORD — never the photo bytes.
  ...
  assertEquals(records[0].path, `/__spike/files/devices/${D}/IMG_0001-photo.jpg`);
  assertEquals(records[0].bytesDrained, "pretend-photo-bytes".length); // consumed, not cancelled
  assertEquals(records[0].observedAtOrigin["authorization"], "Bearer probe-token");
});

Deno.test("spike OPTIONS → 204 with no resumable advertised (same plain-PUT path as the real route)", async () => {
  const { fetchImpl } = storageFake({});
  const res = await createApp({ config: CONFIG, fetch: fetchImpl }).request(
    `/__spike/files/devices/${D}/IMG_0001-photo.jpg`,
    { method: "OPTIONS" },
  );
  assertEquals(res.status, 204);
  assertEquals(res.headers.get("Allow"), "PUT, OPTIONS");
