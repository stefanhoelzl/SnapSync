...
import app.snapsync.compose.NoSystemUi
 ...
 *
 * [NoSystemUi] has no platform, so it reaches only the state in which nothing is opened. It has no share
...
class HandoffContractBindingsTest {
    ...
    private val none = object : Binding<LinkOpenerState, SystemUi> {
        ...
        override fun create(state: LinkOpenerState, clauseId: String): Entered<SystemUi> = when (state) {
            LinkOpenerState.UNCLAIMED -> Entered.Ready(NoSystemUi)
