 ...
 *    `snapSyncProcess` for the per-process event ports (`CrashReporter`, `ProcessMetrics`), which every root sets up
 *    before it composes anything, and from `snapSyncExtension` for the upload extension's `ExtensionHost`: that
 *    process has no host zone at all.
 *
    ...
    fun `handlers are built only in the composition and host zones`() {
        ...
        assertEquals(
            setOf(
                ...
                "/domain/compose/src/commonMain/kotlin/app/snapsync/compose/ExtensionCore.kt",
    ...
    fun `listen is called only from the host zone`() {
        ...
        assertEquals(
            setOf(
                ...
                "/domain/compose/src/commonMain/kotlin/app/snapsync/compose/ProcessComposition.kt",
                // The upload extension's one entry port, registered by its own composition — it has no host zone.
                "/domain/compose/src/commonMain/kotlin/app/snapsync/compose/ExtensionCore.kt",
            ),
            sites,
