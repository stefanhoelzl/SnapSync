...
| 12 | behavior: liveness poll · Swift transcriber · `ProtectedData` never created | shells → ~1 | ● (Session D PASSED on-device: rawValues 0/1/2 SDK-confirmed, PROCESSING honored live; Swift rewrite compiled first try; poll/lifecycle/fan-out/absence oracles green; warm-return + AASA tap operator-gated. Measured: beacon 22→17 — modules 4→3 [`:domain:keychain` deleted] · shells 18→14 [swift 4→0 keyword decisions; kt 14 unchanged]; Swift pin table → one `??` — the `PHBackgroundResourceUploadProcessingResult(rawValue:)` construction, Swift-only type per proof ①, expiry: re-evaluate at iOS 27 GM ~Sept 2026 with the async extension protocol. Δ-note ①: ① resolved as Kotlin-decides-raw-Int (`processingResultRawValue`, ports/, exhaustive `when` pinned in commonTest at failure=0/processing=1/completed=2) — values DERIVED from the swiftinterface case order, **Session D verifies them against the SDK before merge**. Δ-note ②: the silent-push `.noData`-for-missing-eventId distinction dropped (Swift always reports `.newData`) — a case the backend cannot send (`backend/src/apns.ts` `silentBody` always includes `eventId`); D4 of `foreground-poll-and-swift-transcriber`. Δ-note ③: the unlock-hook repair replaced by a trigger-time membership re-read (`AppPorts.reloadConfig`, first step of Foreground/SilentPush/DownloadBackstop, keep-last-good on unreadable via `configAfterReload`) — strictly wider coverage; what is given up is defer-and-**resume** for the pre-first-unlock wake, priced at zero by ④ (zero deferrals in all production logs); the wake now fails cleanly at the adapters (unreadable ≠ absent) and converges at the next trigger. Δ-note ④: MainViewController's transient-error choreography NOT absorbed (Kotlin/presentation-side, orthogonal) — re-assigned to step 13b. Poll cadence 2 s = the sync-status latency bound; poll seated in feature/status, lifecycle ordered by the Foreground/Background flows) |
| 13a | rename `:app:ios:photokit-extension` → `:app:ios:extension` (+ pbxproj, `ios-archive`, `appShellSources`) | modules Δ | modules Δ (measured: beacon 17→15 — modules 3→1, only `:capability:push` remains for 13b; shells unchanged at 14, the extension adapter's 6 decisions moved with the scan. Pure R100 ×5 + string ride-alongs; ExtensionSafetyTest re-rooted with red-proof; framework identity SnapSyncUploadKit untouched, D1. The 3 pbxproj edits are Linux-blind — the final device pass's Xcode archive is their proof) | ● |
