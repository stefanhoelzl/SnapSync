...
class UploadTransitionsTest {
    ...
    private class World(
        ...
        /** The grant, as the permission port publishes it — read by the transitions at every access. */
        private val grantCell = MutableStateFlow(grant)
        ...
        val transitions = UploadTransitions(
            configSource = membership(joined = joined, unreadable = unreadable),
            photoAccess = galleryAccess(grantCell),
...
/**
 * The real membership over the files port: joined to `E`, or not joined — or, when [unreadable], a membership file the
