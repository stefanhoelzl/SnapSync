...
The manifest version — the counter the ledger's triggers advance on every change the device manifest projects
from, which orders the app's and the upload extension's publishes at the backend — and its `11.sqm` migration came
from `changes/archive/2026-09-22-manifest-versions`.
...
Decision record for the walk memo that answers an unchanged library: `changes/archive/2026-09-25-own-work-per-wake`.
...
a fresh batch (decision record: `changes/archive/2026-09-25-own-work-per-wake`, D13).
...
it becomes that **every walk's answer** is one (decision record: `changes/archive/2026-09-25-own-work-per-wake`, D9, amending
