...
extension (`changes/archive/2026-09-22-both-uploaders-active`), each publish carrying the ledger's manifest version so the
backend stores a publish only when it is not older than the one it holds (`changes/archive/2026-09-22-manifest-versions`) —
publishing synchronously in-cycle as a per-event
