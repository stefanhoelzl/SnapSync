...
Implementations MAY be a single object implementing both ports, but consumers SHALL depend on each port separately. These contracts are consumed by the `setup-gate` capability, which renders the permission step of the setup gate and routes its intents.
