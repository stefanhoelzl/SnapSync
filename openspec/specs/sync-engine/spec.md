...
The shared, stateless decision core of the sync backend: platform adapters drive it with events
(a resource changed, an upload failed) and execute the upload jobs it answers with. The sync
domain knows only resources — asset handling lives in a later layer above the seam; encoding and
