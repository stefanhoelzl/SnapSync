...
use-case stops the upload producer, clears the upload ledger (the current membership's share set) and forgets
the `eventId`, returning to the setup gate — without touching anything already uploaded to storage or the
download store's imported rows. A later join reloads the ledger from the device's stored-file listing, so
