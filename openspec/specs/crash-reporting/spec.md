...
crash-surviving report context, and the measurements behind them): `changes/archive/2026-09-14-add-os-exit-attribution`; every outgoing event bounded below the ingest's maximum size by per-part caps (why a refused event blocks
the queue, the caps, their measured sum): `changes/archive/2026-09-23-bound-diagnostic-event-size`.
