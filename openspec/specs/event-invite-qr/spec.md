...
join capability — the `eventId` in the Keychain — so it re-encodes the same `snapsync://` join
deeplink (`encodeConfigUrl`) and displays it as a scannable QR ("Scan to join this event") with
