...
Decision record: `changes/archive/2026-08-25-record-uploads-in-database` (`design.md` for the decisions,
`PROBE-FINDINGS.md` for the measurements each one rests on),
`changes/archive/2026-09-07-adopt-bunny-cli-migrations` (the migration mechanism: ordered checksummed
`.sql` files applied by the platform runner, the generated created form, the in-file SQL precondition,
the `SELECT *` ban, and why migrations apply with foreign-key enforcement genuinely off).
