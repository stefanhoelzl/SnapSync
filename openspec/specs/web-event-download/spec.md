# web-event-download Specification
...
assembles each asset's primary medium into a zip **in the browser** for a one-click download — with no
app, no account, and no service worker. The event reads it depends on are un-attested by
