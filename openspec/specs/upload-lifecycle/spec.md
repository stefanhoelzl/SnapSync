...
Decision record for triggers delivered to the mechanism, which holds none of its own: `changes/archive/2026-09-25-own-work-per-wake`.
...
`changes/archive/2026-09-25-own-work-per-wake`, design D1): each wake does its own work, and the tail then runs the remaining work in
