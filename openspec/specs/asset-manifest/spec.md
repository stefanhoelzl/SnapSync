...
The per-asset manifest is a small JSON document that makes each backed-up asset self-describing and
whole-knowable: it declares the asset's identity, capture timestamp, and the complete set of
**original-only, immutable** resources the asset is expected to have, each typed by a generic,
platform-neutral `role`. Because only originals are listed and originals never change, an asset's
resource set is fixed at capture, so a consumer can determine completeness from the manifest alone
without enumerating storage or knowing the producing platform. The manifest is uploaded out of band
(not an engine `Resource`, not a ledger row) with no ordering guarantee relative to its resources.
Authoritative design: docs/design.md §3.1 (keys), §4 (storage).
