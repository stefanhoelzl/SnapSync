...
The iOS workflow's **merge gates** SHALL be exactly the two parallel jobs `ios-build` and `ios-test`, and these are the only two iOS status-check contexts required by the branch ruleset on `main`. Adding a `needs:` dependency between them is forbidden: a failing `ios-test` would then *skip* `ios-build`, whose required check would never be posted, freezing merges.
...
dev build loop invocation (dev infrastructure; see the `ssh-mac-build` skill), never a CI input. Where the
...
- **WHEN** an operator builds against a declared backend on the dev build loop
