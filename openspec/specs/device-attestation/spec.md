...
## Purpose
TBD - created by archiving change add-device-attestation. Update Purpose after archive.
## Requirements
...
Exactly four things SHALL be reachable without a token, and the list SHALL be closed — a route not named
here SHALL require the token:
