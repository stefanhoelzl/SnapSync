...
Xcode 26.6; decision record: `changes/archive/2026-08-09-delete-simulator-session-downgrade`) — so the
downgrade this requirement previously provided for defended nothing. **Whether the OS relaunches a
terminated app to deliver `handleEventsForBackgroundURLSession` on a simulator is NOT evidenced by that
