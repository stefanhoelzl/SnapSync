...
driven by the app's **tail runner** — the one opportunistic tail every OS wake hands its leftover work to, woken by
the session's relaunch drain and a `BGProcessingTask` heartbeat — over the same shared `UploadCycle` as the
OS-driven tier.
...
The tail runner necessarily reimplements what the OS gives the other tier for free — scheduling, backpressure,
...
`changes/archive/2026-09-25-own-work-per-wake` (why the retired `BackgroundUploadPump` became the tail runner:
each OS wake does its own work, then hands the rest to one tail that stops only on the operating system's
expiry).
...
"The tail runner reimplements the OS scheduler"; decision record `changes/archive/2026-09-25-own-work-per-wake`, design D1). A push
...
subject only to the `SKIPPED` rule of "The tail runner reimplements the OS scheduler". Decision record: `changes/archive/2026-09-25-own-work-per-wake` (design D1, D3, D4, D5).
...
Decision record: `changes/both-uploaders-active` (D5, D6); `changes/archive/2026-09-25-own-work-per-wake` (the tail runner).
...
time, not held forever (decision record `changes/archive/2026-09-25-own-work-per-wake`). That outcome SHALL be **predicted rather than diagnosed**: the process
...
Decision records: `changes/both-uploaders-active` (D7); `changes/archive/2026-09-25-own-work-per-wake` (design D2 — a freed slot
...
slot costs no library enumeration at all (decision record `changes/archive/2026-09-25-own-work-per-wake`, design D2).
...
of a behaviour-preserving change: decision record `changes/archive/2026-09-25-own-work-per-wake`, design D13.)
...
work cooperatively at the next boundary"; decision record `changes/archive/2026-09-25-own-work-per-wake`, design D1). This
...
entry re-reads them anyway (decision record `changes/archive/2026-09-25-own-work-per-wake`, design D11; it replaces the retired
