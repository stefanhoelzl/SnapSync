...
The filename derivation used to seed SHALL be the single shared library-enumeration derivation that
the upload producer also uses, so a seeded `COMPLETED` row's key matches a later `ResourceChanged`
for the same resource. Because an uploaded resource is immutable, the engine treats any `COMPLETED`
