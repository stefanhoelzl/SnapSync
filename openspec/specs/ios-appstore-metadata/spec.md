...
(description, keywords, promotional text, support URL, marketing URL) **and its screenshots** — so the
listing is version-controlled, reviewable in a PR, and reproducible, instead of hand-typed and
hand-uploaded into the App Store Connect (ASC) web console and living only there. The one listing field
this capability deliberately does **not** commit is **"what's new"**: it differs for every release, so
it is derived per release from the labelled pull requests since the previous one and written by the
