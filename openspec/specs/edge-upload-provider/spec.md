...
(`/event/<eventId>/device/<deviceId>/file/<filename>`) using only string-building — no crypto, no
signing, no network I/O. It sets `Content-Type` only and carries the deterministic, injective
...
`<host>/event/<eventId>/device/<deviceId>/file/<encoded-filename>`, where `host` is the injected
compile-time base, and `<eventId>`/`<deviceId>` are injected verbatim (already canonical UUIDs,
not re-encoded). `<encoded-filename>` SHALL be the percent-encoding of `resource.filename`'s UTF-8
...
- **WHEN** `provide` returns
- **THEN** `url` is `<host>/event/<eventId>/device/<deviceId>/file/<encoded-filename>` with no
  `?`-query parameters

### Requirement: Plain-string configuration contract

The provider SHALL take its three placement inputs — the edge `host`, the `eventId`, and the
`deviceId` — as injected **plain strings**, with no BuildKonfig dependency and no platform API
calls inside the provider. (Sourcing those strings — host from the bundle, eventId from the
Keychain, deviceId from the App Group — is the consuming composition root's responsibility, not
the provider's.)

#### Scenario: Built from literal strings
- **WHEN** an `EdgeUploadRequestProvider` is constructed with literal `host`/`eventId`/`deviceId`
...
for the same `(host, eventId, deviceId, resource.filename)` SHALL yield a byte-identical URL, so a
