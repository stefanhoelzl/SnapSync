...
(the re-join seed timestamps its rows with the join time, and uploaded resources are immutable).
Exposing `url` is **not** a leak of the storage key: `url` is the backend's own public route, stable
because the backend owns it and revealing nothing about the storage backend, whereas the storage key
