...
5`, with the JSON body `{ "aps": { "content-available": 1 } }` (a silent push — no `alert`, `sound`, or
`badge`). The sender SHALL rely on the runtime `fetch`'s automatic HTTP/2 negotiation and SHALL NOT
require a bespoke HTTP/2 client library or a native dependency. A push SHALL carry only the transport
discriminator's `kind == "apns"` tokens; a non-`apns` token SHALL be skipped.

#### Scenario: A silent background push is posted

- **WHEN** the sender pushes to an `apns` token with `env` `production`
- **THEN** it `POST`s to `https://api.push.apple.com/3/device/<token>` with `apns-topic`,
  `apns-push-type: background`, `apns-priority: 5`, and body `{ "aps": { "content-available": 1 } }`
