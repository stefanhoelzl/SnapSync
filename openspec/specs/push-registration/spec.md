...
foreground discovery and the `BGProcessingTask` heartbeat, both of which remain correct if no push ever
arrives.

Decision records: `changes/archive/2026-07-05-push-notification-infra`; the need-named `PushTokenPublisher` port and
its contract, `changes/archive/2026-09-23-device-credential-contracts`; a push's own work (the download reconcile) before the
wake's one tail, and its handler held until that work ends, `changes/archive/2026-09-25-own-work-per-wake`.
## Requirements
...
`deliver`. Decision record: `changes/archive/2026-09-25-own-work-per-wake` (D12).
...
about (decision record: `changes/archive/2026-09-25-own-work-per-wake`, D1).
...
whose time is the scarcest the app has (decision record: `changes/archive/2026-09-25-own-work-per-wake`, D12).
...
as a healing net was declined for its cost (decision record: `changes/archive/2026-09-25-own-work-per-wake`, D12).
