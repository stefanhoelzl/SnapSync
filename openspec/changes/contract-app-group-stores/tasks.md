...
- [ ] 1.1 In the kexe, measure which unreadable entry at a file path — a mode-000 file, or a directory —
...
- [ ] 1.2 In the kexe, measure whether a per-test `NSUserDefaults(suiteName:)` round-trips a value and can
...
- [ ] 2.1 `FileBackedConfigStore`: add `containerPath: String?`, defaulting to the App-Group lookup, and
  derive the file path from it. Both shells stay unchanged.
- [ ] 2.2 `IosStagedBytes`: add `container: () -> String?`, defaulting to the lookup and still resolved
  lazily inside `stagingRoot()`. The shell stays unchanged.
- [ ] 2.3 `./gradlew build` and `compileIosMainKotlinMetadata` are green, and `RuntimeIdentityTest`'s
  pinned literals are unmoved.

## 3. Config contract and the `clear()` fix

- [ ] 3.1 `:test:contracts` commonMain: `ConfigStoreState` (`INACCESSIBLE`, `ABSENT`, `JOINED`, `FOREIGN`,
...
- [ ] 4.1 `DeviceManifestStoreContract` (`UNAVAILABLE`, `EMPTY`, `HOLDING`):
  ...
  - the live binding is `IosDeviceManifestStore` in ext-safe iosTest, where `UNAVAILABLE` uses the default
    container.
- [ ] 4.2 `StagedBytesContract` (`UNAVAILABLE`, `EMPTY`, `STAGED`), with paths derived from `stagingRoot()`
  and the clause id:
  - the fake binding is in `:adapter:generic:fake`;
...
- [ ] 4.5 `ContractCoverageTest` is green: every new clause has a `Live` binding on `IOS_SIM_KEXE` declaring
  its state.

## 5. The world on the fake

- [ ] 5.1 `:test:world`: replace the inline `configSource`/`configStore`/`configReader` objects with one
...
- [ ] 5.2 `:test:integration`, `:app:desktop` (the world inspector's lever) and `CycleEntryGateIntegrationTest`
  are green. Any test that saved while unreadable is fixed at the test.

## 6. Docs, diagrams, gates

- [ ] 6.1 CLAUDE.md module map:
...
- [ ] 6.2 `./gradlew architectureDiagrams`, and commit anything it changes.
- [ ] 6.3 `./gradlew build` is green on Linux. The `iosSimulatorArm64Test` suites of `:adapter:ios:ext-safe`,
  ...
  table has no `Failed`.
- [ ] 6.4 `npx --yes @fission-ai/openspec@1.5.0 validate --specs --strict` and `… validate
