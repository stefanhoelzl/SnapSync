...
- [x] 6.2 ANSWERED, and then CORRECTED once the mechanism was measured. Every transfer fails instantly with `NSURLErrorDomain/-1` — loopback AND LAN — while the default session in the same process reaches the same server and curl fetches the identical presigned URL with 200. The first reading ("the session RUNS, only the transfers fail — inert in outcome, alive in mechanism") is **withdrawn**: the app-side pipeline runs, but the REMOTE session is never created. `nsurlsessiond` resolves each client's bundle identifier and drops the connection when it is `(null)`, which it is for every process we can build — including a real installed app declaring a valid `CFBundleIdentifier`. Hence `NSCocoaErrorDomain 4097` = `NSXPCConnectionInterrupted` (accepted then torn down), not `Invalid`/4099. Six candidate fixes tested, none works
- [x] 6.3 Supersession recorded in the design record, on corrected terms: `fix-download-session-lifecycle` D5's closing limitation holds in outcome AND in mechanism, and its premise ("the simulator cannot run background sessions") is **correct for third-party code** — the opposite of what an intermediate revision of this record claimed. Archive untouched. Two consequences are NAMED but not taken here, being behaviour-touching: `IosUrlSessionUploadPlatform`'s comment is false, and `2026-08-09-delete-simulator-session-downgrade` D1 is falsified
