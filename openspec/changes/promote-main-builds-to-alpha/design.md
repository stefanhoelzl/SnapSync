...
- **The job is only ever exercised on `main`.** Being `main`-only, it cannot run on the PR that introduces it. → **Resolved before merge.** The four `main`-only guards were temporarily relaxed onto a throwaway branch (`8c962a7`, reverted by `c88e1a7`) and the full chain `ios-build → ios-deliver → ios-promote` was run for real, producing build 318: it landed in `alpha`, `BETA_APPROVED`, `autoNotifyEnabled: false`, and a re-run of the job correctly skipped every mutation step. This is strictly better than the existing convention (`ios-deliver` has only ever been exercised on `main`) and the trick is worth reusing.
