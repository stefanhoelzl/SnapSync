...
render the join QR (with its caption) above the hero and the flat icon-only
share action in the bottom action cluster. The caption's wording is owned by the screen and its tests
(capability `event-invite-qr`); the harness SHALL render whatever the screen renders and SHALL NOT
pin the text. The harness SHALL NOT perform a real platform share — the
...
- **THEN** the phone frame renders a scannable join QR with its caption and a
