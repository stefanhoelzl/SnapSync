...
- **The job is only ever exercised on `main`.** Being `main`-only, it cannot run on the PR that introduces it. → **Resolved before merge.** The four `main`-only guards were temporarily relaxed onto a throwaway branch commit and the full chain `ios-build → ios-deliver → ios-promote` was run for real ([run 29353145285](https://github.com/stefanhoelzl/SnapSync/actions/runs/29353145285)), producing build 318: it landed in `alpha`, `BETA_APPROVED`, `autoNotifyEnabled: false`, and a re-run of the job correctly skipped every mutation step. The throwaway commit was then **dropped from history, not merely reverted** — the repo allows only rebase-merge, so a revert pair would have put a commit titled *"TEMP: DO NOT MERGE"* on `main` permanently. This is strictly better than the existing convention (`ios-deliver` has only ever been exercised on `main`) and the trick is worth reusing.
- ~~**Apple's processing wait is an unmeasured estimate.**~~ **Measured, and the estimate was wrong.** The design assumed minutes of idle polling and justified the Ubuntu runner partly on keeping that idle off macOS. In reality `ios-promote` completed its work in **8 seconds**: build 318 was discoverable on the *first* lookup and already `VALID` **96 seconds** after upload. There is effectively no processing wait for an app this size. → The retry caps (10 min to find, 20 min to process) are left deliberately slack: they cost nothing on a fast day and are the only guard against a slow day at Apple turning a run red. The Ubuntu choice still stands on its own merits (the job needs no Apple toolchain at all), but not on the idle-runner argument.
