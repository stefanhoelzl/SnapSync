...
- [x] 10.4 Post-release FOLLOW-UP (not a completion condition; archived at the user's decision): after release, watch Bugsink dumps (`/bugsink`) for the OS-expiry lines and imports left staged — the field evidence (iPhone XS, iOS 18) this change rests on

## 11. Sync and archive

- [x] 11.1 Fix the stale Purpose texts a delta cannot edit, during sync: `ios-url-session-upload` ("pumped by `BGProcessingTask`", "The pump necessarily reimplements…" → the tail runner); `sync-status` ("off the pump's critical path"); `coverage-bounds` (names the deleted `DownloadBackstop` flow — add a note); `push-registration` ("the `BGProcessingTask` backstop" → heartbeat)
- [x] 11.2 Archive gates (2026-09-25): (1) no placeholder Purpose anywhere; (2) delta completeness — domain/{model,ports,feature,flow,compose}, adapter/ios/{app-only,ext-safe}, app/{composition,ios,ios/extension}, iosApp, test/{world,rig,architecture,contracts,integration} are covered by the 21 deltas (ios-app-shell, module-architecture, photo-download, ios-url-session-upload, upload-lifecycle, push-registration, sync-status, sync-ledger, ios-photokit-upload, diagnostic-logging, limited-photo-access, photo-selection-policy, edge-upload-provider, device-attestation, sync-engine, architecture-guards, harness-world-model, testing-architecture, join-event, reconfigure-membership, upload-state-reconciliation); no delta needed for adapter/generic/app (download-store: the batched plan/mark port members are behaviour-preserving and the spec names none of them; its contract clauses carry them), adapter/generic/fake (honest doubles of changed ports, held by the port contracts), test/control (typed-client field rename, protocol unchanged), ui/presentation (comment only), the stdio non-blocking fix (no log output changes in a SpringBoard launch; design D15), .claude/.github/CLAUDE.md (docs); (3) dead types — every spec still naming `DownloadBackstop`, `ReceiptDeadlines`, `OsReceipt`, `BackgroundEventsReceipts`, `BackgroundUploadPump` states it as retired/removed; `UploadPushReceiver` and the rest are named in no spec
