...
- [x] 1.6 Replace the feature lambdas for `now`, `permission`, `joined`, `activeEventId` and `activeConfig` with the existing ports (`Clock`, `PhotoAccessStatusSource`, `ConfigSource`), and change `SecureStore.resolveOrMint` to take `legacy: SecureStore?`.
- [x] 1.7 Add the `UserQueries` bundle to `model/` (`loadJoinDetails`, `shareableCount`) and build it in `compose/` with the `awaitingOnCoreLane` decorator. `StatusContainerHost` takes it and runs the count as an intent reducing `Counting | Count | Unavailable`. Delete `ShareCountRow`'s effect query and `ParticipationActions.shareableCount` (fixes B6).
...
- [x] 2.4 ~~Add a `@ParityFor("<entry>")` integration test in `:test:integration` for each OS entry point in `SnapSyncRoot`: foreground, background, open-url, push token, silent push, upload heartbeat, download backstop, and background-URLSession relaunch. Each starts from a cold core.~~ *Superseded on rebase:* main's `shell-as-driving-adapter` added `PlatformEntriesContract`, which drives every OS entry point through the inbound port from a fresh World, with coverage enforced by `ContractCoverageTest`. Now that the world boots cold (2.3), that contract IS the cold-core parity test; the duplicate tests were dropped. The two-process B1 relaunch test stays.
- [x] 2.5 ~~Add the entry-point parity gate.~~ *Superseded on rebase:* the `Shell` delegate is gone (main moved the OS entry points into `compose/`'s `platformEntries`), and `ContractCoverageTest` already requires every `PlatformEntriesContract` clause to run.
...
- [x] 3.3 Add the catch gate, with an allowlist of the helpers, the ObjC-boundary helpers and `runProcessCycle`.
...
- [x] 6.2 Correct the stale spec text the review found outside this change's deltas: `leave-event` and `event-invite-qr` still describe `leave`/`share` as lambdas injected into `StatusContainerHost`. *Done as deltas: each "container … action defaults to a no-op" requirement is REMOVED and replaced (leave) or folded into the modified share requirement (share), since the actions now arrive through the required `UserCommands` bundle. At sync, also fix `event-invite-qr`'s Purpose by hand — it still says "`share: (String) -> Unit` lambda (no-op default)", and a delta cannot carry Purpose text.*
- [x] 6.3 Run the three archive gates in `openspec/config.yaml` before archiving.
