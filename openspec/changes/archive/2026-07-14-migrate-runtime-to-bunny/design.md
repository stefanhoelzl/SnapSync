...
## Appendix — the retired `backend-config` capability

`backend-config` is **deleted**, not emptied: its requirements fold into `backend-deployment` (Decision
6). OpenSpec deltas cannot express "remove a capability" — a spec with zero requirements fails
validation — so the capability is removed as a file and its rationale is preserved here, verbatim from
the delta that could not be applied.
