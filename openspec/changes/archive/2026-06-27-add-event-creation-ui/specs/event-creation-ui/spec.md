...
`201 { eventId, name, createdAt }` funnels the returned `eventId` into the **existing** provision path
— the same `onProvision(previousEventId, newEventId)` switch-reset and `ConfigStore.save` a scanned
deeplink uses — so the app auto-joins with no new join path. On any failure (non-2xx, transport, or
...
#### Scenario: Successful create provisions the event
- **WHEN** `create("My Party")` is invoked and the backend returns `201` with an `eventId`
- **THEN** the event is provisioned through the same `onProvision` + `ConfigStore.save` path as a
  scanned QR, config becomes present, and the existing join/reconcile flow runs
...
- **THEN** the create proceeds (mints + provisions) without inspecting permission, and the missing
  permission surfaces afterward via the existing `UiState.PermissionBlocked` path
