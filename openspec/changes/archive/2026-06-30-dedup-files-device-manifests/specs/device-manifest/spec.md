...
array). Each `resources` element SHALL carry `role`, `contentType` (the resource's MIME type),
`filename` (the resource's object name — its `/files/<deviceId>/` storage key minus that prefix), and
`originalFilename` (the resource's human filename as captured).
...
- **AND** each resource entry carries `role`, `contentType`, `filename`, and `originalFilename`
