...
**Reason**: Its only job was to detect, inside a short-lived upload cycle, a membership change nobody
announced, using the `joinedEventId` marker. Every change of membership is now an explicit app action (a
join, a switch, or a leave), because a reinstall leaves no membership; the ledger is set at that action
instead of being detected afterwards.

**Migration**: Replaced by "A join loads the ledger from the per-device listing" (the load runs in the app
at a first join or a switch) and "The upload cycle does not detect membership changes". The marker, the
`UploadReconciler` and the cycle's `reconcile` port are deleted; the orphaned `rejoin.joinedEventId` key is
removed once on process start (capability `ios-app-shell`).

### Requirement: Reconciliation seeds already-stored resources as completed

**Reason**: The seed no longer runs in the upload cycle and no longer sets a marker. Its substance — an
atomic `resetTo` to one bare `COMPLETED` row per listed resource, with a successful listing authoritative
even when empty — moves to the join, where a failed fetch clears the ledger instead of deferring.

**Migration**: See "A join loads the ledger from the per-device listing".

### Requirement: Event switch versus re-join

**Reason**: The marker comparison it specifies is deleted, and its leave rule (keep the ledger, clear only
the marker) is reversed: a leave now clears the upload ledger (capability `leave-event`), and a switch is a
leave followed by a join, whose load replaces the ledger (capability `sync-ledger`, "The ledger is the
current membership's share set"). Dedup survives both, because the next join's load restores every stored
resource from the per-device listing.

**Migration**: The re-provision no-op becomes the join-time load's "A re-provision of the joined event loads
nothing" scenario. The manifest re-projection for a new event is owned by capability `device-manifest`.

### Requirement: Upload tier defers uploads until the seed succeeds

**Reason**: A failed listing fetch no longer defers anything: the join-time load clears the ledger and
uploading proceeds, at the stated cost of idempotent re-uploads bounded by the event window. The
`SeedDeferred` cycle outcome is deleted.

**Migration**: See "A failed load blocks nothing". The 15-second bound moves to the join-time load.

### Requirement: A read-only foreground check reports upload state the backend contradicts

**Reason**: `UploadLedgerAudit` read the `joinedEventId` marker this change deletes, and its "unlisted" count
has been wrong since the walk started deleting rows of assets that left the library (its log line claimed
the next walk would re-upload them, which is false for those). The detector's question — does the backend
lose rows the ledger records as landed — is not re-homed.

**Migration**: None. `UploadForeground.check` is deleted and `UploadForeground` collapses to the pump.
