...
The sync domain SHALL define `SyncStatus(pending, completed, failed, active, estimatedRemaining: Duration?, lastFinishedAt: Instant?)`, observed through the unchanged `SyncStatusSource` contract. Counts describe the most recent pass (in-flight or finished). The sync domain SHALL expose a computed `state` property as the single source of truth for classification:
