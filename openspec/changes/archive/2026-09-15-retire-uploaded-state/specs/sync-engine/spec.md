...
**Reason**: No platform submits `UploadCompleted`. Both upload tiers record a successful upload into the
ledger themselves, where the platform reports it, through the guarded `markTerminal` (capability
`sync-ledger`); the engine's completion arm and `LedgerWriter.recordCompleted` had no production caller.
**Migration**: `SyncEvent.UploadCompleted` is deleted. A platform records success with
`markTerminal(key, COMPLETED)`; a `COMPLETED` key is still skipped by "Resource-changed decision".
