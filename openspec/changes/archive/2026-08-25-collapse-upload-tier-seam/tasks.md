...
- [x] 2b.3a **Measured and reported** (SE2, iOS 26.6, 2026-08-25): with PHOTOKIT pinned under a `LIMITED` grant, **both** halves are refused with `PHPhotosErrorAccessUserDenied` — `extension enable FAILED: PHPhotosErrorDomain:3311`. So `ios-photokit-upload`'s *"registration succeeds and lies"* is false as stated; the app cannot register at all there. Sent to `limited-grant-registration-noise` with the raw lines and the caveat that n=1 through a pin does not retroactively prove what the 2026-07-20 probe saw. Recorded as design D11b
...
- [x] 7.3 **Verified on device (SE2, iOS 26.6, 2026-08-25) via the pin, under a FULL grant** — the case a `LIMITED` downgrade cannot reach because the platform refuses the operation there. With the extension registered, pinning the app-driven mechanism ran `photokit.deregister` (the narrow verb) and it **succeeded**: *"extension disable succeeded — a configuration record existed and was removed"*, followed straight by `url-session.start` with no `clearRequested` and no cursor reset. Three minutes earlier the same producer's `photokit.start` → `photokit.stop` DID run the repair path — same producer, two verbs, chosen by what runs next. Ledger unchanged across the relinquish (63 completed / 0 pending); `osExtension.enabled` true→false with `grantDependent: false`. **Not covered:** a `REQUESTED` row surviving *in flight* — the seeded uploads complete in under a second, so that window could not be constructed. The carve-out is instead shown by which verb runs, and `deregister()` provably touches no ledger state
