...
- [x] 1.1 Add `adapter/generic/app/src/commonMain/sqldelight/ledger/app/snapsync/engine/db/10.sqm`:
...
- [x] 1.2 In `Ledger.sq`, remove the three columns from `CREATE TABLE`, and remove them from every statement:
  `get`, `recordUnlessSettled`, `insert`, `selectByDestinationPath` and `selectNeedingJob`.
- [x] 1.3 In `Ledger.sq`, delete the statements for the removed sweeps: `backfillEventId` and
  `clearAbsenceMarks`.
- [x] 1.4 In `Ledger.sq`, remove the `absent = 0` filters from `selectManifestRows`, `selectPending`,
  `aggregates` and `selectNeedingJob`.
- [x] 1.5 Retarget `demoteRequested` to `SET state = 'DISCOVERED'`.
- [x] 1.6 Rewrite every comment in `Ledger.sq` that names a removed concept. This covers the header, the column
  notes, the `get` note, the guard note ("a stale FAILED"), `selectRequestedKeys` and the `markTerminal` note.
- [x] 1.7 Leave `ledger/databases/9.db` as it is (design D6). Confirm that `verifyCommonMainLedgerDatabaseMigration`
  passes.

## 2. Model: three states, no attempt, no provenance, no mark, no `UploadJob`

- [x] 2.1 In `domain/model/.../Ledger.kt`, remove `LedgerState.FAILED`.
...
- [x] 2.2 From `LedgerEntry`, remove `attempt`, `eventId` and `absent`, and remove `markedAbsent`/`markedPresent`
...
- [x] 2.3 In `domain/model/.../SyncModel.kt`, delete `UploadJob` (design D4).
...
- [x] 2.4 Remove the `filterNot { it.absent }` read in `DeviceManifest.kt`.

## 3. Ports and adapters

- [x] 3.1 In `LedgerStore`, delete `backfillEventId` and `clearAbsenceMarks`. Reword the KDoc of
...
- [x] 3.2 Update the `TransferRecord.markTerminal` KDoc: the recordable set is `COMPLETED`/`DISCOVERED`, and the
...
- [x] 4.4 Update `placeFirstEnqueued`'s KDoc per design D5. Its predicate does not change.
- [x] 4.5 In `StrandedKeys` and `DemoteRequested`, update KDoc to say "recorded `DISCOVERED`". No logic changes
  (phase 5 owns the repair's structure).
- [x] 4.6 In the `Reconciler` seed, construct the `LedgerEntry` without `attempt`/`eventId`. Nothing else in the
  file changes (design D7).
- [x] 4.7 Remove the `filterNot { it.absent }` read in `AlbumGather.ownSet`.
- [x] 4.8 Update every `SyncEngine(...)` construction in `compose/`, `:test:world` and tests, which now take no
  `eventId`.
- [x] 4.9 Run `./gradlew compileIosMainKotlinMetadata` and `./gradlew build`.

## 5. Tests

- [x] 5.1 Add the `SqlDelightLedgerStoreTest` migration case from design D6 (v10 → v11: the `FAILED` rewrite,
...
- [x] 5.2 In `:test:world`'s `LedgerStoreContract`, `LedgerRecordGuardContract` and `LedgerSeeds`:
...
- [x] 5.3 In `domain/feature` commonTest, update the two test-local ledger fakes (`InMemoryLedgerStore`,
...
- [x] 5.4 Add an `UploadCycleTest` case: a row that returned to `DISCOVERED` after a failure is placed again by
  the enqueue pass (design D5), replacing the old "re-created failure is not placed" test.
- [x] 5.5 Update `:test:integration`, `:test:world` world tests, and `:app:desktop`'s world inspector wherever
  they name `FAILED` or a ledger/engine attempt. The world's own creation counter stays.
- [x] 5.6 Run `grep -rnE "\bFAILED\b" --include=*.sq --include=*.sqm --include=*.kt` over the tree. Every
