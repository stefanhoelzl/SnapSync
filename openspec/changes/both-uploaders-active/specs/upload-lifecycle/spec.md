...
**Reason**: Both uploaders may run, so there is no longer a "which single mechanism runs" question to resolve.
`resolveUploadMechanism` / `UploadMechanism` are deleted; each process's creation is its own admission, and the
one fact the transitions still need is whether the extension may be registered.

**Migration**: See ADDED "Whether the extension may be registered is one pure fact", which carries over the
rules that survive: never a trapping selector below iOS 26.1, presence ≠ runnability, the transport binding is
not an input, and the app engine is constructed once per process. The per-process admission is in "The upload
cycle owns its entry decision".

### Requirement: Exactly one mechanism writes the ledger, enforced at each engine's entry gate

**Reason**: The premise is removed. The ledger already had writers in both processes, and enforcing one writer
is what made every hand-off between the uploaders cancel or orphan in-flight work that then needed repair.
Both uploaders are active; safety comes from write-after-act and the guarded one-transaction writes
(`sync-ledger`), and an overlap is an accepted duplicate upload.

**Migration**: See ADDED "Both uploaders may run; an overlap is a duplicate, never a loss". The app's
not-resolved decline is replaced by admission under any usable grant ("The upload cycle owns its entry
decision"); the pin that deregistered the extension is the per-uploader switch ("A mechanism override is a
