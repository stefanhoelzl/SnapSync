...
### D10 — Delivery folded into the `ios-build` job; main-only steps (supersedes D6)
Rather than a separate `ios-release.yml` or a separate `ios-release` job, the device app is built by a **single `ios-build` job**, to avoid compiling `iosArm64` twice on `main`. On non-`main` refs it builds **unsigned** (the cheap merge gate — no secrets, signing-independent); on `main` the same job instead **archives-signs-and-uploads** (the signed archive recompiles `iosArm64`, so it stays the gate). Chosen over making the gate always-signed (the rejected "Option 1": one `ios-release` job that signs on every push) because that would make every PR gate depend on the ASC key / Apple signing availability and force a ruleset rename of the required `ios-build` context. Accepted trade-off: on `main`, delivery does not `needs: ios-test` — but the commit already passed `ios-test` on its PR before merging, so re-gating is redundant (the `ios-test` check still runs on `main` in parallel). The build number derives from the **`ios` workflow's** `run_number` (a higher counter than the old `ios-release.yml`), still monotonic, so TestFlight build numbers jump up once but never collide.
