...
- [ ] 1.8 Pass `QueuedPhotoDownloadJobs.onStaged` as a constructor parameter that resolves `downloadController` when invoked. Delete the `var` and the `?: return` (fixes B1), and do the same for `MetricKitProcessMetricSource.onReport` and `SnapSyncRoot.uploaderPinSource`.
...
- [ ] 1.13 Add a regression test that builds `AppCore`, touches only `downloadJobs`, stages a resource through the fake transport, and asserts that the controller marked it staged and imported it.
