...
**Reason**: The app no longer reads the ledger; status is derived from storage truth (the completeness
listing), the on-disk manifests, and the gallery total.
**Migration**: Replaced by the listing-backed source below; the `LedgerWatcher` and
`ObservedCompletionsSource` inputs are dropped.
