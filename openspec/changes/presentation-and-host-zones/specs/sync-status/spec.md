...
read (on the presentation side, the containment is mechanical: the presentation zone has no edge to
`ports/`, and the read-model import gate confines its `feature/` references to `readmodel` packages —
capability `architecture-guards`, "The zone gates").

The presentation zone (`:domain:presentation`) SHALL consume status only through the
`SyncStatusSource` seam, which is declared in `feature/status/readmodel`, and the feature's read-model
types — never a ledger type, a port, or the engine.
