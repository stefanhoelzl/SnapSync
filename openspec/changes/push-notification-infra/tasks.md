...
- [x] 1.1 Add `APNS_KEY_ID`, `APNS_TEAM_ID`, `APNS_PRIVATE_KEY`, `APNS_TOPIC` to `Config` and the
  `readConfig` fail-closed inventory (throw naming any missing/blank var), alongside the storage vars.
- [x] 1.2 `deno test`: `readConfig` throws when each APNs var is absent/blank; succeeds when all present.

## 2. APNs sender (`backend/src/apns.ts`)

- [x] 2.1 Implement ES256 JWT signing with WebCrypto from the `.p8` PEM (header `{alg:ES256,kid}`,
  claims `{iss,iat}`); cache/reuse the signed token, re-signing when older than one hour.
- [x] 2.2 Implement `sendSilent(tokens)`: per token, select host from `env` (`production`→
...
- [x] 2.3 Isolate per-token failures — record each token's outcome (status/skip), never throw out of the
...
- [x] 3.1 Add `PUT /devices/:deviceId/config` (deviceId UUID-gated, ungated by event): write the JSON
...
- [x] 3.2 Add `POST /event/:eventId/notify`: marker gate (`404` absent / `502` non-404 read failure);
...
- [x] 3.3 Update the `app.ts` header route/layout comments and `backend/README.md` for the two new
  routes and `config.json`.
- [x] 3.4 `deno test`: config-write happy path + key + faithful `5xx`; notify gate `404`/`502`,
...
- [x] 4.1 Create the module (`capability/push/build.gradle.kts`, `settings.gradle.kts` include;
  `jvm()` + `iosSimulatorArm64`; deps `:capability:device-id`, Kermit, the shared HTTP client seam);
  no third-party push dependency.
- [x] 4.2 `commonMain`: `PushTokenSource` seam (token + `env`, rotation, settable test fake) and
  `PushReceiver` seam (silent-push receipt); `env` is an injected compile-time value.
- [x] 4.3 `commonMain`: `PushRegistration` use-case — build+send `PUT <host>/devices/<deviceId>/config`
...
- [x] 4.4 `commonTest` (JVM + `iosSimulatorArm64`): registration PUTs the exact URL+body; failed write
  is absorbed; no eventId anywhere; re-register same token is idempotent.
- [x] 4.5 `iosMain`: adapters — acquire the APNs token via `UIApplication` registration (feeding
  `PushTokenSource`) and a `PushReceiver` impl that logs receipt (Kermit).

## 5. iOS app shell wiring (`:app:ios`, `iosApp/`)

- [x] 5.1 `iosApp/iosApp/iosApp.entitlements`: add `aps-environment` (`development`/`production` by
  build config); `iosApp/iosApp/Info.plist`: add `remote-notification` to `UIBackgroundModes`.
- [x] 5.2 `iosApp/Configuration/Config.xcconfig` → `Info.plist`: bake `APNS_ENV`
  (`sandbox`/`production`) per build configuration; expose it to Kotlin for `PushTokenSource`.
- [x] 5.3 `iOSApp.swift` `AppDelegate`: `registerForRemoteNotifications` on launch; forward
...
- [x] 5.4 `SnapSyncRoot` (`app/ios/src/iosMain`): compose `:capability:push` into the live root —
...
- [x] 6.1 `:test:world` MiniEdge/BackendStore: serve `PUT /devices/<id>/config` (store `config.json`)
...
- [x] 6.2 `:test:integration`: assert a device registration writes the config document, and that a
