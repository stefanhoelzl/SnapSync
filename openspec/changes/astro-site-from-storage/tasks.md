...
- [x] 5.3 First cutover: deploy `site/` to storage before/independently of the api proxy change so `/` resolves; verify `GET /` returns the Astro page through the pull zone, `no-cache` on HTML, `immutable` on assets — **CI-automated on merge**: `site-deploy.yml` runs the mirror-deploy (storage-key secret) and `api-deploy.yml` ships the proxy; a green `site-deploy` run confirms the upload landed. The runtime observation (`curl /` + header check) is a trivial post-merge eyeball — there is deliberately no post-deploy probe (`backend-deployment`)
