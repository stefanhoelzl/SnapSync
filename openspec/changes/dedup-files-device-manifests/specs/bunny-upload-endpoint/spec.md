...
**Reason**: Photo-byte uploads are now **ungated**. Bytes are event-independent — stored under
`files/<deviceId>/` and reused across events by reference — so a per-byte event-existence check on the
hot path is wrong (a byte may legitimately precede or outlive any single event). The cheap
event-existence gate moves to the infrequent per-cycle device-manifest write (`PUT
/event/<eventId>/device/<deviceId>`), which keeps it to avoid orphan manifests.

**Migration**: Accepted abuse trade-off for a personal TestFlight app — any client with the
IPA-baked edge host can write to `/files/<deviceId>/` and the device id is self-asserted; App Attest
(or similar) is the future hardening path (design §8). No data migration: old `<eventId>/<file>`
objects are orphaned under the clean cutover.
