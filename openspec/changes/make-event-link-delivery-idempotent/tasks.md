...
- [ ] 1.1 Reset the working branch's **code** to `origin/main`, keeping this change's own
...
- [ ] 1.2 Confirm the merged instrumentation from PR #215 is present on `main`: the scene-delegate
...
- [ ] 2.1 Add the duplicate-delivery rule to the join gate, keyed on the pending join's event and
...
- [ ] 2.2 Record an ignored duplicate with the entry point that delivered it, so it is distinguishable
      from a link that never arrived
- [ ] 2.3 `commonTest` (runs on JVM **and** `iosSimulatorArm64`): same link twice starts one pending
      join and issues one details fetch; a different link supersedes; the same link after commit or
      dismissal is a fresh delivery
- [ ] 2.4 `commonTest`: an `autoJoin=true` link delivered twice provisions **once** — the only path
      that double-provisions today, so the sharpest test of the rule
- [ ] 2.5 `:test:integration` over `:test:world`: assert `UiState` **and** world outcomes for a
      doubled delivery — one enrollment, one membership

## 3. Restore the SwiftUI delivery path

- [ ] 3.1 Add `.onOpenURL` to the `WindowGroup`, forwarding the raw `absoluteString` to Kotlin under
      its own entry-point name; no parsing in Swift
- [ ] 3.2 Add the Kotlin entry point with `@PlatformEntry` and the logging wrapper, routing to the same
      `onOpenUrl` door every other path uses
- [ ] 3.3 Add its rig trigger-inventory entry (a trigger or an exclusion with the reason that makes the
      omission safe)

## 4. Remove the app-delegate continuation trio

- [ ] 4.1 Delete `application(_:willContinueUserActivityWithType:)`,
...
- [ ] 4.2 Delete the three `onApp*` Kotlin entry points and their rig exclusions
- [ ] 4.3 Shrink the `SwiftShellGuardTest` pin table in the same commit, so it never overstates the debt

## 5. Guards

- [ ] 5.1 Extend `EventLinkDeliveryTest` to pin `.onOpenURL` and its forwarding, with a failure message
...
- [ ] 5.2 Verify the guard is non-vacuous: it fails when the modifier or its forwarding is removed
- [ ] 5.3 Confirm the guarded Swift sources are declared as inputs of the guard's test task, or the
      guard silently stops re-running when its subject changes

## 6. Correct the record

- [ ] 6.1 Rewrite the falsified comment block in `iosApp/iosApp/iOSApp.swift` — `.onOpenURL` "never
...
- [ ] 6.2 Update `SnapSyncRoot`'s KDoc on the scene-continuation entries to the "app already running"
      framing, dropping the warm-vs-cold and link-source framings that the measurements retired
- [ ] 6.3 Check no other prose in the repo still claims the SwiftUI modifier cannot receive a universal
      link, or that iOS 18 cannot do continuations at all

## 7. Verify and ship

- [ ] 7.1 `./gradlew build architectureDiagrams` green; `openspec validate --specs --strict` green
- [ ] 7.2 Dispatch the branch (`gh workflow run ios.yml --ref <branch>`) and verify **on device**,
...
- [ ] 7.3 Confirm from a diagnostic dump that duplicates are logged and ignored rather than silently
      absent
