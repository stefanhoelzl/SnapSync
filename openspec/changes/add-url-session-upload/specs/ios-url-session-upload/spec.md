...
The iOS 18–26.0 **app-driven** upload tier. Where iOS provides no PhotoKit background-upload
subsystem, the main app performs uploads itself over a background `URLSession`, reusing the shared
`:capability:upload` orchestration (`UploadCycle`) behind a second `UploadJobPlatform` implementation.
It is selected per OS version and coexists with the iOS ≥26.1 `ios-photokit-upload` tier.
