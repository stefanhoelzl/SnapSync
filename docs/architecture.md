...
| The photo library is one thin port: `GalleryReader` (both processes: assets by policy or id, resources by id, albums, album members, create, add) and `Gallery` (the app: plus the access request, the selection picker, the partial grant's selection observer, the import and the change token). It answers what the platform shows, and `NotReadable` — never an empty answer — when no grant lets the process read. Whether a walk is authoritative for deletion, which grant may say a photo is gone and which albums are denied are the gallery services' (`services/`). Asset ids cross as `AssetId`s, opaque to the core | `GalleryReaderContract`, `GalleryContract`, `GalleryImportContract` (live on `IOS_SIM_APP`, the no-grant state on `IOS_SIM_KEXE`) + `GalleryServicesTest` |
| A photo's identity is one canonical `AssetId` (`model/`): non-empty, RFC 3986 unreserved characters only, never `..` — a URL path segment the backend accepts as it is, an object-name fragment, and free of the download tag's `\n`. The core, both stores, the manifest and the backend carry it unchanged; there is no translation on either side of the wire. Only a platform adapter maps its native id to it, reversibly — PhotoKit's `{UUID}/L0/NNN` maps `/`↔`_` in `PhotoKitAssetIds`, and an identifier with no exact mapping is skipped with an `Error` line, never guessed. A value from outside (a backend reply, an OS-held task tag) that is not canonical is a malformed reply or an undecodable tag, never a crash | construction (`AssetId`'s `init`) + `AssetIdTest`, `PhotoKitAssetIdsTest` (`IOS_SIM_KEXE`) and the gallery contracts, whose seeded ids are minted through the adapter's own mapping |
...
{ "deviceId": "<uuid>", "version": 42,
  "assets": [ { "assetId": "<canonical AssetId>", "creationDate": "<ISO-8601>",
                "resources": [ { "role": "primary", "contentType": "image/heic",
                                 "key": "<assetId>-primary.heic", "filename": "IMG_0001.HEIC" } ] } ] }
```

- **`assetId`** is the device's canonical `AssetId` (section 2), sent and stored as it is. The iOS form is the
  PhotoKit identifier with `/` mapped to `_` — the same bytes every build since v1 has sent, so no stored row, object
  name or ledger key changed when the type was introduced.
